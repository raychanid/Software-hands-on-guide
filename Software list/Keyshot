一：我们首先从打开软件自带的一个DEMO查看一下效果。从启动的界面选择一个场景，在这里我选择的是相机模型的场景。
二：左边的是材质库，环境库，背景库，纹理库等，方便我们快速选择软件预设好的使用
三：右边的是项目中实时渲染及最终渲染的各方面参数设置，都可以影响到最终的效果
四：在这里是设置渲染的质量及分辨率，图片大小，文件格式等。
五：点击菜单栏的渲染或者快捷键ctrl+P对当前场景进行渲染。看质量大小耐心等待一会，便可看到一张产品级别的图片了
六：接着我们从其它软件导入模型进行渲染。点击工作窗口下方的导入按钮，选择一个我们在MAYA或者3DMAX中制作好的OBJ格式文件或者FBX等见的3D格式文件，在这里我使用的是FBX格式文件，导入一个战舰模型。在右边的项目设置中设置模型的纹理路径，材质类型便可实时在场景中显示出整个战舰模型的效果。
七:设置好渲染输入路径，材质，环境后，就可以ctrl+p对模型进行渲染了。耐心等待，渲染完成。
八：keyshot的快捷键：
...
其实还可以用塑料（高级），塑料（模糊），半透明，绝缘材质也一样的可以调出本文两个例子的材质。高级材质中还有个氛围参数没说到，这个参数可以用来做一些半透明或全透明的夜光材料。也可以做到有自发光材质一样的效果。 
	...
	【级别】及【级别调整】是克隆现有的笔刷，再把它摆放到附近。功能原理与上面的【噪点】同样原理。
	这个动动手试一试就基本清楚了，试之前要把密度调小一点，要不然你可能分不出它到底克隆了那一个是那一个。
	
	【拉丝】有着很丰富的调整参数，可塑性比较强。熟练了之后，各个参数配合调节可以造出很多的变化，可应用到各个场景当中。
	---------------------------------------------------------------------------------------------------------------- 
	现在再新建一个（【噪点】碎形），双击它再按下C键。数值默认的情况下你可能会得到下图这样子
	
	出现过的参数都不用说了。
	【衰减】它在这里外于颜色的下面，这里的衰减不同于其他地方的，是控制颜色1和2。默认值是1.2，往高的调是颜色1占比多一点，往低的调是颜色2占比多一些。动动手试一下。
	【级别】这个是为笔刷增添细节的。默认值为5，每档为1的整数，但是在5之后好像变化不大。
	下面是1，5，10的比较
	
	
	
	好了！噪点还是比较简单的，而且（【噪点】纹理）的调整方式也几乎是一样的，大家新建一个试一下就明白了。OK,现在偿试一下调整出下图的效果吧，像下图那样子就差不多了。
	不管你用碎形还是纹理，在很细很细的纹理表现时我感觉效果相差不太大，起码在这个例子里是这样吧。
	
	好了！现在看下图：
	
	还记得上一节说过的小技巧吗？
	现在我们有两个贴图要叠加连接上凹凸通道。完成好是下图那样子，而且你还可以预先新建一个【色彩渐变】连上【凹凸添加】的【几率】节点上。因为紧接着就该它出场了。
	
	
	看上图：
	这是【色彩渐变】调整之后的效果和参数。两个边边的灰度渐变会让纹理有个渐消的过程，不致于突然断掉。
	【凹凸添加】和【色彩渐变】在上一节已经研究过了，相信不用说大家都知道该怎么弄了。
	现在重点说一下为什么会让【色彩渐变】连到【比率】上面，因为上一节没有合适的案例细说，就草草带过了。
	【比率】默认值0.5，些时两个贴图的输入与输出相当于直通。当值为0时，只有凹凸1影响材质，而当值为1时，只有凹凸2可以起作用。
	实际上就是不透明度在影响，想让谁来显示出一个怎样的显示结果。
	当【色彩渐变】贴图接管了【比率】之后，则两个【重量】就会完全遵守黑透白不透的规则，只在该出现的地方出现了。
	这个时候如果你想再调整两路【重量】显示比例的的权重，那就去独立去调他们自己的参数好了。
	
	好了！到现在也基本完成了。完成效果如下：截屏图，请无视那些噪点。
	
	上面我们一步步的模拟了现实中的刀具的生产加工工艺，步骤你们可以更改一下，比如调好一个贴图就连上去【凹凸添加】看看效果再进行下一步。其实我也是这样做的，只不过是为了方便编写排版而这样写罢了。
	
	
	我来说一下关于光影和质感的关系。
	首先是光影，KS里的光影操作方式类似一个球形HDR文件，这和3DMAX里创建一个球形天空，去影响地面上物体的光影和反射一样。那么KS里也是同理，相当于把产品放在了一个圆形的物体中间，我们在圆形物体上去增加和改变颜色，亮度，渐变，位置，从而去塑造中间的产品。理解了这个概念，那么我们能用KS里的HDR编辑工具能做什么了？能做到什么地步了？这需要同学们自己去创造了，KS提供给我们可创造的东西，特别是更新的KS7，远远比我们想的多的多。
	　
	首先是光影，KS里的光影操作方式类似一个球形HDR文件，这和3DMAX里创建一个球形天空，去影响地面上物体的光影和反射一样。那么KS里也是同理，相当于把产品放在了一个圆形的物体中间，我们在圆形物体上去增加和改变颜色，亮度，渐变，位置，从而去塑造中间的产品。理解了这个概念，那么我们能用KS里的HDR编辑工具能做什么了？能做到什么地步了？这需要同学们自己去创造了，KS提供给我们可创造的东西，特别是更新的KS7，远远比我们想的多的多。
	
	上面就是我的HDR文件，在整个HDR文件中，我只用到了5个位置的光。也就是说这6个光就能产生如下图的光感和质感。也就是说渲成这样，就能满足我在PS里后期强化的要求。这也是一项需要长期学习才能达到的一项技能。那就是后期强化术之PS。
	
	　
	接下来分析一下这5个光分别是什么作用。经过我的后期强化术之QQ截图后得到下图。
	
	点亮2和3号位置的光，来确定整个产品的主光源方向。这也是三大面之一：亮面。光打到这里，首先要说明的是，在这个产品表现中，我不需要如上图一般的死黑面，所以我要竟可能的去呈现亮面和灰面。所以接下来我的光影思路是去体现产品的结构同时提亮暗部。
	　
	
	
	点亮1和4的灯光，得到上图。那么现在出现的问题是：整个产品因为过多的亮面和灰面，产品变得缺少层次。某些地方转折并不明显。所以我需要去增加产品的明暗对比度，增强转折的明暗关系。
	　
	
	
	　
	
	点亮5和强化了3号位灯，解决了上面所提到的一些问题。当然整个光影还可以继续优化。最后经过后期强化术得到最终图。（如下图）
	
	　
	
	总结：渲染产品就和画画一样，一定要有三大面，五大调的思路在里面。这样出来的光影本身才具有层次，干净，影调。和产品与材质结合以后才会容易出来质感。最后用到后期强化术一幅KV图就是水到渠成。最后想说的是不要依赖软件本身的HDR，和网络上各种HDR文件，这会对你整个思路利大于弊。一定要主动的去创造HDR文件，其它各种室内室外的HDR只是给你创造的HDR添砖加瓦，主次一定要清晰。
